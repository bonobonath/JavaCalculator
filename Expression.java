...
public class Expression {
	 ...
	 * (this initializes mTokenQueue) Declare and create a Tokenizer object named
	 * tokenizer passing pExprStr to the ctor Declare a Token object named prevToken
	 * initialized to null -- Read the first token Declare a Token object named
	 * token and assign the return value from tokenizer.nextToken() to it -- Keep
	 * reading tokens until tokenizer.nextToken() returns null While token is not
	 * null Do -- Check for and handle the negation operator. If token instanceof
	 * SubOperator Then token = negationCheck(token, prevToken) End if -- Add the
	 * token to the queue. Call getTokenQueue().enqueue(token) prevToken = token --
	 * Read the next token. token = call tokenizer.nextToken() End While
	 */
	 ...
	 * instanceof LeftParen Then Push token onto the operator stack (type cast token
	 * to LeftParen) ElseIf token instanceof RightParen Then While the operator on
	 * the top of the operator stack is not an instanceof LeftParen Do Call
	 * topEval(operatorStack, operandStack) End While Pop the top operator from the
	 * operator stack -- removes the LeftParen Else Declare Operator object named
	 * operator and assign token to it (type cast to Operator) While
	 * keepEvaluating(operatorStack, operator) is true Do Call
	 * topEval(operatorStack, operandStack) EndWhile Push operator onto the operator
	 * stack End If End While While the operator stack is not empty Do Call
	 * topEval(operatorStack, operandStack) End While Pop the top Operand from the
	 * operand stack and return its value (call getValue() on the Operand).
	 */
	 ...
	 * pOperandStack.pop() Declare and create Operator object named operator = Call
	 * pOperatorStack.pop() If operator instanceof UnaryOperator Then Typecast
	 * operator to UnaryOperator and call evaluate(right) on it Push the returned
	 * Operand from the above statement onto the operand stack Else Declare and
	 * create Operand object named left = Call pOperandStack.pop() Typecast operator
	 * to BinaryOperator and call evaluate(left, right) on it Push the returned
	 * Operand from the above statement onto the operand stack End If
	 */
